...
class GAN(object):
    ...
    def __init__(self, sess, epoch, batch_size, dataset_name, checkpoint_dir, result_dir, log_dir,learningRateD,learningRateG):
        self.sess = sess
    ...
    def build_model(self):
        ...
        self.next_steps=tf.placeholder(tf.float32)
        self.Times=tf.placeholder(tf.float32)
        self.restart_step=tf.placeholder(tf.float32)
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            #                                                     first_decay_steps= 4*(tf.ceil(self.restart_step)))
            #
            # self.learning_rate = tf.train.cosine_decay_restarts(learning_rate=0.001, global_step=self.global_step,
            #                                                     first_decay_steps = 6*500,t_mul=2,m_mul=0.9,alpha=0.5)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.num_batches):
                ...
                #                                              feed_dict={self.inputs: batch_images, self.z: batch_z, self.global_step:epoch*500})



                # _, summary_str, d_loss,lr= self.sess.run([self.d_optim, self.d_sum, self.d_loss, self.learning_rate],
                #                                          feed_dict={self.inputs: batch_images, self.z: batch_z, self.global_step:epoch,self.restart_step:re_step})
    ...
    def visualize_results(self, epoch):
        ...
        for i in range(50):
            image_path = 'D:/test11111111111111/test11111111111111/images_Ablation/deer_original' + '/' + self.model_name + 'number%03d' %i
